...
export default {
	...
	setMarkerClickHandler(subType = '', item, param) {
		if (subType === '') {
			return
		} else if (subType === 'ky_sb_pcs' || subType === 'ky_sb_local_pcs') {
			if ($vm.drawFlag || $vm.rangingFlag) return
			$vm.detailMarker && $vm.detailMarker.remove()
			this.getPcsClick(item, param);
		} else {
			if ($vm.drawFlag || $vm.rangingFlag) return
			$vm.detailMarker && $vm.detailMarker.remove()
			...
			// Popper弹框图层
			console.log("sssssssssssss", subType)
