...
export default {
	 ...
	 * @param subType  图层小类
	 */
	filterLayer(mainType = '', subType = '', curGaArea = 'ALL') {
		if (mainType === '' || subType === '') {
			return
		}
		// mainType = "ky_sb"
		...
		this.data.configs[mainType][subType][curGaArea] = config_ga[mainType][subType][curGaArea];
	...
	createLayer(source = [], mainType = '', subType = '', curGaArea = 'ALL') {
		...
		if (source.length === 0 || mainType === '' || subType === '') {
			...
			source.map((item) => {
				...
				// let gaType = this.gacType.[item.UNIT_NAME];
				let gaType = item.GAC;
				console.log(gaType);
				if (item.GPS_POINT_Y !== '' && item.GPS_POINT_X !== '') {
					...
					let marker = L.marker({
						...
						icon: L.divIcon({
							className: 'marker-keyun-container ga-marker sub-type-' + subType + ' ga-type-' + gaType, // 还没改
							data: item,
