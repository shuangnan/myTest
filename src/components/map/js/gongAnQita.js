...
export default {
    ...
    filterLayer(mainType = '', subType = '') {
        ...
        console.log("$$$$$$$$$$$$$$$filterLayer: this.data.configs[mainType][subType] = config_ga[mainType][subType];", this.data.configs[mainType][subType])
        console.log("$$$$$$$$$$$$$$$filterLayer: this.data.layers[mainType][subType]", mainType, subType, this.data.layers[mainType][subType])
            ...
            source.map((item) => {
                ...
                if (item.GPS_POINT_Y !== '' && item.GPS_POINT_X !== '') {
                    ...
                    marker.on('click', (p) => {
                        ...
                        // if (subType === 'wu_yxjg_hosiptal') {
                        //     this.setMarkerClickHandler('ky_sb_yl', item, p);
                        // }
                    })
    ...
    setMarkerClickHandler(subType = '', item, param) {
        if (subType === '') {
            ...
            if (subType.includes('wu_yxjg_')) {
                if (subType === 'wu_yxjg_school') {
                    item.NAME = item.SCHOOL_NAME;
                } else if (subType === 'wu_yxjg_tlnb') {
                    item.NAME = item.STN_NAME
                }
                subType = 'ky_sb_yl' // 为了popper的样式统一，之后再改
